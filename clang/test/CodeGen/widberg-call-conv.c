...
// RUN: %clang_cc1 -triple i386-pc-linux -emit-llvm -fwidberg-extensions < %s

void __fastcall f1(void);
void __usercall f2(void);
...
void __usercall f5(void) {
  // CHECK-LABEL: define{{.*}} usercallcc void @f5()
  f2();
  // CHECK: call usercallcc void @f2()
}
...
void (__usercall *pf2)(void) = f2;
void (__thiscall *pf3)(void) = f3;
void (__fastcall *pf4)(void) = f4;
void (__usercall *pf5)(void) = f5;
...
// PR7117
void __usercall f7(foo) int foo; {}
void f8(void) {
  f7(0);
  // CHECK: call usercallcc void @f7(i32 0)
}
