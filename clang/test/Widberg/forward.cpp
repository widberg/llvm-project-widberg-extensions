...
int __usercall baz@<eax>(int a1) { // expected-error{{conflicting types for 'baz'}}
    return a1;
}
// expected-note@9{{previous declaration is here}}

void __usercall qux(int a1@<ecx>) { // expected-error{{conflicting types for 'qux'}}
    return;
}
