...
X86RegisterInfo::getCalleeSavedRegs(const MachineFunction *MF) const {
  ...
  case CallingConv::PreserveMost:
    return adjust_regmask(CSR_64_RT_MostRegs_SaveList);
  ...
  case CallingConv::HHVM:
    return adjust_regmask(CSR_64_HHVM_SaveList);
...
X86RegisterInfo::getCallPreservedMask(const MachineFunction &MF,
  ...
  case CallingConv::PreserveMost:
    return adjust_regmask(CSR_64_RT_MostRegs_RegMask);
  case CallingConv::PreserveAll:
    if (HasAVX)
  ...
  case CallingConv::HHVM:
    return adjust_regmask(CSR_64_HHVM_RegMask);
  case CallingConv::X86_RegCall:
    if (Is64Bit) {
      if (IsWin64) {
  ...
  if (Is64Bit) {
    ...
    return adjust_regmask(IsWin64 ? CSR_Win64_RegMask : CSR_64_RegMask);
  }
